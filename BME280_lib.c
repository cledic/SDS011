...
int BME280_Write( uint8_t*buffer, int length);
int BME280_Read( uint8_t address, uint8_t*buffer, int length);
 ...
 */
int32_t BME280_Init( const char *device )
{
	float p, t, h;
	...
	ret = BME280_Read( BME280_REG_ID, &buffer[0], 1);
...
int32_t BME280_Get_Temperature( float*t)
{
	...
	ret = BME280_Read( BME280_REG_TEMP_MSB, &buffer[0], BME280_TEMP_SZ);
...
int32_t BME280_Get_Pressure( float*p)
{
	...
	ret = BME280_Read( BME280_REG_PRESS_MSB, &buffer[0], BME280_PRESS_SZ);
...
int32_t BME280_Get_Humidity( float*h)
{
	...
	ret = BME280_Read( BME280_REG_HUM_MSB, &buffer[0], BME280_HUM_SZ);
...
int32_t BME280_GetRawValues( uint32_t*press, uint32_t*temp, uint32_t*hum)
{
	...
	ret = BME280_Read( BME280_REG_PRESS_MSB, &buffer[0], BME280_PRESS_SZ+BME280_TEMP_SZ+BME280_HUM_SZ);
...
int32_t BME280_GetStatus( int32_t*s)
{
	...
	ret = BME280_Read( BME280_REG_STATUS, &buffer[0], 1);
...
int32_t BME280_ReadCalibrationData( void)
{
	...
	ret = BME280_Read( BME280_REG_CALIB1, &buffer[0], BME280_CALIB1_SZ);
	ret = BME280_Read( BME280_REG_CALIB2, &buffer[BME280_CALIB1_SZ], BME280_CALIB2_SZ);
...
int32_t BME280_SetTempOversampling( int32_t to)
{
	...
	ret = BME280_Read( BME280_REG_CTRL_MEAS, &b[1], 1);
...
int32_t BME280_SetHumOversampling( int32_t ho)
{
	...
	ret = BME280_Read( BME280_REG_CTRL_HUM, &b[1], 1);
...
int32_t BME280_SetInactiveDuration( int32_t id)
{
	...
	ret = BME280_Read( BME280_REG_CONFIG, &b[1], 1);
...
int32_t BME280_GetSensorMode( int32_t*sm)
{
	...
	ret = BME280_Read( BME280_REG_CTRL_MEAS, &value, 1);
...
int32_t dump_Register( void)
{
	...
	ret = BME280_Read( 0xF2, &reg_values[0], 4);
	return ret;
}
